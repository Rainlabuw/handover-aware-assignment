...
from ..algorithms.solve_w_haal import solve_w_haal, solve_w_haald_track_iters
from ..algorithms.solve_w_centralized_CBBA import solve_w_centralized_CBBA
from ..algorithms.solve_w_CBBA import solve_w_CBBA, solve_w_CBBA_track_iters
from ..algorithms.solve_greedily import solve_greedily
from ..algorithms.classic_auction import Auction
...
def compare_MHA_to_other_algs():
    ...
    axes[0].bar(["Naive", "SGA", "MHA", "HAAL2", "HAAL5"],[no_handover_ben, sga_ben, mha_ben, mhal2_ben, mhal5_ben])
    axes[0].set_title("Average benefit across 10 runs")
    # axes[0].set_xlabel("Lookahead timesteps")

    axes[1].bar(["Naive", "SGA", "MHA", "HAAL2", "HAAL5"],[no_handover_nh, sga_nh, mha_nh, mhal2_nh, mhal5_nh])
...
def performance_v_num_agents_line_chart():
    ...
    for n in ns:
        ...
        for _ in range(num_avgs):
            ...
            _, mhal_val, mhal_nh = solve_w_haal(benefits, init_assign, lambda_, L)
            mhal_total_vals += mhal_val/num_avgs
            mhal_total_nhs += mhal_nh/num_avgs

            _, mhald_val, mhald_nh = solve_w_haal(benefits, init_assign, lambda_, L, distributed=True)
    ...
    axes[0].plot(ns, mhal_vals, label=f"HAAL (L={L})")
    axes[0].plot(ns, mhald_vals, label=f"HAAL-D (L={L})")
    ...
    axes[1].plot(ns, mhal_nhs, label=f"HAAL (L={L})")
    axes[1].plot(ns, mhald_nhs, label=f"HAAL-D (L={L})")
...
def tasking_history_plot():
    ...
    mhal_ass, mhal_val, _ = solve_w_haal(benefits, init_assign, lambda_, 1, graphs=None)

    mhal5_ass, mhal5_val, _ = solve_w_haal(benefits, init_assign, lambda_, 6, graphs=None)
    ...
    axes[1].plot(range(T), agent1_mhal_ass, label="HAAL 1")
    axes[2].plot(range(T), agent1_mhal5_ass, label="HAAL 5")
    ...
    plt.plot(range(T), mhal5_valid_tasks, label="HAAL")
    plt.legend()
    plt.show(block=False)
    ...
    val_ax.plot(range(T), mhal_val_line, 'b', label='HAAL')
    val_ax.plot(range(T), mhal5_val_line, 'g', label='HAAL 5')
...
def paper_experiment2_tasking_history():
    ...
    # axes[0].plot(range(T), agent1_no_handover_ass, label="Not Considering Handover")
    # axes[1].plot(range(T), agent1_mhal5_ass, label="HAAL-C")
